                    ...
                    if numSubArray == 0:
                        ...
                        if weight.shape[1] == 3:
                            ...
                            for k in range (int(bitWeight/self.cellBit)):
                                ...
                                remainderQ = remainderQ + torch.normal(0., torch.full(remainderQ.size(),self.vari, device=remainder.device))
                            ...
                            for z in range(bitActivation):
                                ...
                                for k in range (int(bitWeight/self.cellBit)):
                                    ...
                                    # the range of remainder is [0, cellRange-1], we truncate it to [lower, upper]
                                    remainderQ = (upper-lower)*(remainder-0)+(cellRange-1)*lower   # weight cannot map to 0, but to Gmin
                                    remainderQ = remainderQ + remainderQ*torch.normal(0., torch.full(remainderQ.size(),self.vari, device=remainder.device))
